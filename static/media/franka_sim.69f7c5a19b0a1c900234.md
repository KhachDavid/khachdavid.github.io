...
The vision behind the project is to be able to run libfranka programs by passing a simulation server instead of the real robot ip address. With the backend running in Drake, the simulation is able to support complex dynamics. This makes it possible to run franka programs in a simulated environment with the same fidelity as the real robot, with few exceptions, of course. 
...
<p style="
    ...
    color: #777;
">
Block Diagram: Simplified architecture showing Franka FCI and Drake FCI communication via TCP/UDP with libfranka interface and optional MoveIt/ROS2 integration.
</p>
</br>
...
This ability to conduct precision control with such high accuracy opens the door to a lot of robot learning applications. A great next project would be to use the simulator to train a robot to perform a task.
